...
class DartAPI:
    ...
    def find_corp_code(self, corp_name):
        ...
        # 4. 완전히 실패(상장사 중 매칭 없음) → RAG(임베딩 기반 유사도 검색) 시도
        try:
            model = SentenceTransformer('jhgan/ko-sroberta-multitask')
            names = df_clean['corp_name'].tolist()
            embeddings = model.encode(names, convert_to_tensor=True)
            query_embedding = model.encode(corp_name, convert_to_tensor=True)
            cos_scores = util.pytorch_cos_sim(query_embedding, embeddings)[0]
            best_idx = cos_scores.argmax().item()
            best_score = cos_scores[best_idx].item()
            best_name = names[best_idx]
            best_code = df_clean.iloc[best_idx]['corp_code']
            best_stock_code = df_clean.iloc[best_idx]['stock_code']
            # 유사도가 0.7 이상이고 상장사라면 자동 반환
            if best_score > 0.7 and best_stock_code and best_stock_code != '':
                return best_code
            else:
                return f"입력하신 기업명과 유사한 기업이 있습니다: {best_name} (유사도: {best_score:.2f})"
        except Exception as e:
            return f"입력하신 기업명을 찾을 수 없습니다. 한글/영문으로 바꿔서 입력을 다시 시도해 보세요. (RAG 오류: {e})"
